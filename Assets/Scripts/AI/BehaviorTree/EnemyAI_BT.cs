...
public class EnemyAI_BT : MonoBehaviour
{
    ...
    {
        root.Evaluate(); // Tick the behavior tree every FixedUpdate
        MoveForward();
        if (rotating) HandleRotationStep();
    }
    ...
    private void UpdateObstacleInfo()
    {
        ...
        // Decide rotation based on ray hits
        if (hitForward)
        {
    ...
    private void BuildBehaviorTree()
    {
        ...
        Node checkObstaclesNode = new ActionNode(() =>
        {
            UpdateObstacleInfo();
            return NodeState.Success;
        });

        // Rotate towards target if not blocked
        Node rotateTowardsTargetNode = new ActionNode(() =>
        {
            int dir = GetRotationDirectionToTarget();
            if ((dir == 1 && blockedOnRight) || (dir == -1 && blockedOnLeft) || dir == 0)
        ...
        // Root Sequence: check obstacles first, then rotate to target
        root = new Sequence(new List<Node>
        {
            checkObstaclesNode,
            rotateTowardsTargetNode
        });
    }
    #endregion
